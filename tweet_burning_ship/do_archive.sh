...
# Delete the image
img_path="$(pwd)/tweet_burning_ship/draft/result.png"
echo "INFO: Deleting '$img_path'."
rm "$img_path"

# Metadata
src="$(pwd)/tweet_burning_ship/draft/metadata.txt"
dst="$(pwd)/tweet_burning_ship/archive/$(date +%s | awk '{printf "%011d\n", $1}')-$TWEET_ID.txt"
echo "DEBUG: src: '$src'."
echo "DEBUG: dst: '$dst'."
